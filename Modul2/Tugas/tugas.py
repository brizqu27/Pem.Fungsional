#1 Buatlah Fungsi add_expense disini
def add_expense(expenses, date, description, amount): return expenses + \
    [{'tanggal': date, 'deskripsi': description, 'jumlah': amount}]

#2 Buatlah fungsi calculate_total_expenses disini
def calculate_total_expenses(expenses, date): return sum(
    expense['jumlah'] for expense in expenses if expense['tanggal'] == date)

#3 Buatlah fungsi get_expenses_by_date disini
def get_expenses_by_date(expenses, date): return [
    expense for expense in expenses if expense['tanggal'] == date]

#4 Buatlah fungsi generate_expenses_report disini
def generate_expenses_report(expenses): return (
...
#6 ubah fungsi berikut ke dalam bentuk lambda
...
if __name__ == "__main__":
    ...
    ]  # Inisialisasi expenses sebagai list kosong
    main()